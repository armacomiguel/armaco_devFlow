...
const TagCard = ({_id, name, questions, showCount, compact, remove, isButton, handleRemove} : Props) => {
  ...
  const Content = (
    <>
      ...
      flex flex-row gap-2 rounded-md border-none
        px-4 py-2 uppercase'>
        ...
        {showCount && <p className='small-medium
        text-dark500_light700'>{questions}</p>}
    </>
  );
  ...
  return (
    <Link href={ROUTES.TAG(_id)} className='shadow-light100_darknone'>
      <article className='background-light900_dark200 light-border flex w-full flex-col rounded-2xl
